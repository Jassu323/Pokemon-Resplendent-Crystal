...
The optional `--ignore` takes a colon-separated address and size of data to ignore in the ROM, so any differences between the `.gbc` and `_vc.gbc` files in that region will not be warned about.
